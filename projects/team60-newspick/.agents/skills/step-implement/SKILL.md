...
7. FE 화면 step이면 `docs/design.md`
8. FE 화면 step이면 step 문서의 `## Prototype 기준`에 적힌 원본 `prototype/` 파일
...
- `docs/design.md`는 보조 색인이다. 원본 prototype과 충돌하면 원본 prototype을 따른다.
- FE 화면 step 문서에 `## Prototype 기준`이 없으면 문서 결함으로 보고하고 구현 전에 중단한다.
- 구현 전에 `## Prototype 기준`의 파일, 섹션, class를 읽고 텍스트, 구조, 색상, 간격, radius, shadow, 상태, 인터랙션을 확인한다.
- 구현 중 임의 단순화, 다른 레이아웃 재해석, 텍스트 변경, 스타일 축약을 하지 않는다.
- Verify 단계에서는 브라우저로 prototype과 구현 화면을 나란히 비교한다. 불일치가 남으면 progress sync, commit, 완료 보고를 하지 않는다.
- 완료 보고에는 prototype과 비교한 브라우저 검증 결과를 포함한다.
...
3. Test: 해당 track의 명령을 실행한다.
   ...
   - BE: Gradle 테스트
   - AI: pytest
4. Verify: UI step이면 브라우저에서 prototype과 구현 화면을 나란히 비교해 step 문서의 수동 검증 항목을 확인한다.
