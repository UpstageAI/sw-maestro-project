...
```text
우리는 24시간 움직이는 가상자산 시장을 계속 지켜보기 어려운 개인 사용자를 위해, Binance Spot Testnet 환경에서 가상 자금으로 현물 주문과 체결 흐름을 안전하게 테스트하고, 자연어로 시작한 매매 의도를 백엔드 세션이 계속 다시 평가해 주며, 설명 가능한 결과를 제공하는 개인용 투자 보조 Agent를 개발한다.
```

### 1.2 서비스 한 줄 정의

```text
Coin Agent는 Binance Spot Testnet의 시세, 호가, 캔들, 잔고, 주문 상태를 조회하고, 사용자가 자연어로 시작한 테스트 의도를 trader style 추론과 함께 해석한 뒤, 사용자가 멈추거나 백엔드 안전 규칙이 멈출 때까지 가상 자금 현물 주문 테스트를 재평가하며, 결과를 자연어와 로그로 설명하는 투자 보조 서비스이다.
```
...
| 항목 | 작성 내용 |
|---|---|
| 주요 사용자 | 개인용 자동화/보조 Agent를 직접 실험하려는 1인 사용자, 그리고 복잡한 거래소 규칙을 쉬운 화면으로 이해하고 싶은 초보 사용자 |
| 사용자가 처한 상황 | 실거래 전 단계에서 Spot Testnet으로 주문·체결·취소 흐름을 검증하고 싶다. |
| 사용자의 목표 | 가상 자금으로 현물 주문 흐름을 안전하게 테스트하고, 각 단계의 결과와 실패 원인을 이해하는 것 |
| 사용자의 어려움 | 인증, 시그니처, 심볼 규칙, 주문 파라미터, 체결 상태를 한 번에 이해하기 어렵고, 자연어 자동매매가 왜 이어지거나 멈췄는지 해석하기도 쉽지 않다. |

### 1.6 핵심 가치

```text
Coin Agent의 핵심 가치는 “실거래 없이도 전체 현물 주문 테스트 흐름을 이해하고 검증할 수 있게 하는 것”이다.
이 서비스는 Binance Spot Testnet만 사용하며, 시세 조회, 잔고 확인, 현물 매수/매도, 주문 상태 조회, 주문 취소, WebSocket 시세 수신, 자연어 기반 연속 판단 흐름까지를 안전하게 실험할 수 있도록 돕는다.
```
...
| 주요 니즈 | 실거래 없는 안전한 주문 테스트, 시그니처/인증 이해, 체결 상태 확인, 실패 원인 파악, 자연어로 시작한 자동매매가 왜 이어지거나 멈췄는지 이해 |
...
이 구조를 Agent라고 부르는 이유는, LLM 호출이 길게 이어지는 선형 Prompt Chaining이 아니라 각 역할이 **자기 상태, 자기 산출물, 자기 실패 기본값**을 가진 채 같은 `run_id` 안에서 이어지기 때문이다. Policy/Planning은 정책과 요청을 grounding하고, Market/Risk는 그 결과를 다시 평가하며, Execution/Report는 실행 또는 차단 결과를 독립적으로 해석한다. 자연어 입력에서는 trader/persona 단서도 함께 읽어 판단 스타일 힌트를 만들 수 있다. 다만 실행 권한은 AI에 있지 않고, 후보 경로 제안까지만 AI가 맡으며 최종 제출 여부는 defensive rule base 와 deterministic 재검증을 가진 BE가 확정한다.
...
| Agent가 도와주는 범위 | 시세 조회, 호가 조회, 캔들 조회, 잔고 조회, 수동 주문 테스트, 자연어 auto order, trader/persona 추론, 연속 자동매매 세션 판단 보조, 주문 상태 조회, 주문 취소, 리포트 생성 |
...
여기서 Agent의 자율성은 임의 매매 권한이 아니라 **사람 개입 없이도 상태를 이어서 판단하고, 필요 시 보류하며, 충분한 근거가 있을 때만 제출 후보를 만드는 능력**을 뜻한다. 이 흐름도 사용자가 멈출 수 있고, 동시에 BE 안전 규칙이 더 먼저 중단시킬 수 있다. 상세 상태 전이와 실행 권한 경계는 `ARCHITECTURE.md`, `AI.md`, `DATA.md`를 기준으로 본다.
...
| 시나리오 4 | 사용자가 "워뇨띠 스타일로 계속 봐줘" 같은 자연어 자동매매를 시작하고 싶다. | 자연어 지시문 | Agent가 `traderId`, `inferredPersona` 를 추론하고, BE 세션이 각 tick 마다 다시 판단한다. | 사용자는 stop 전까지 또는 safety stop 전까지 연속 판단 흐름을 본다. |
...
| 5 | 자연어 auto order | 자연어 지시문을 1회성 agentic run으로 해석하고 제출 후보를 평가 | `rawText` | auto order run 응답, `traderId`, `inferredPersona` | 필수 |
...
6. 조건을 만족하면 `PASS`를 제안하고, 최종 제출 여부는 defensive rule base 와 deterministic 재검증을 가진 BE가 다시 결정한다.
...
- Reports는 현재 run 단위 live 조회라는 점을 유지하고, 시간 누적 history를 이미 제공하는 것처럼 설명하지 않아야 한다.
