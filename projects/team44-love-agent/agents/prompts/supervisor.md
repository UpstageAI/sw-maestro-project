...
사용자의 고민을 분석하고, 4명의 상담 에이전트가 효과적으로 토론할 수 있도록 방향을 제시합니다.  
...
4. **감정 상태 분류**: 사용자가 현재 느끼는 감정 상태를 아래 값 중 하나로 분류합니다.
   ...
   - `"angry"` — 화남·억울함
   - `"neutral"` — 비교적 차분함
5. **토론 목표 설정**: 4개 에이전트가 함께 다루어야 할 핵심 질문을 1문장으로 설정합니다.
...
1라운드에서 4개 에이전트가 제출한 의견을 검토하고, 2라운드 토론의 방향을 정리합니다.  
...
- `diverging_points`는 에이전트 이름을 포함해 구체적으로 기술한다. (예: "지옥에서 온 바람둥이 vs 행동파 연쇄고백마: 거리두기로 가치를 유지할지 즉시 행동할지")
...
1. **합의 항목**: 4개 에이전트 중 3개 이상이 동일하거나 호환 가능한 입장을 보이는 쟁점을 추출합니다.
