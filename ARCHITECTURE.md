...
- **MVP 기준 제안**으로 AI 서비스를 BE 내부 프로세스로 둘지, 별도 서비스로 분리할지 확정이 필요하다.
- **MVP 기준 제안**으로 시세 캐시 계층을 MVP에 포함할지 확정이 필요하다.
- **MVP 기준 제안**으로 야간 자동 감시를 배치성 작업으로 구현할지, 사용자가 요청 시뮬레이션 방식으로 처리할지 확정이 필요하다.
