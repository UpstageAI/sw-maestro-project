...
- **MVP 기준 제안**으로 AI를 HTTP 호출만 사용할지, 내부 gRPC도 함께 구성할지 확정이 필요하다.
- **MVP 기준 제안**으로 policy versioning을 MVP에서 저장만 할지, 롤백 기능까지 넣을지 확정이 필요하다.
- **MVP 기준 제안**으로 Upbit 응답 캐싱을 기본 포함할지 확정이 필요하다.
