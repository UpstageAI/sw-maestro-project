...
이 문서는 현재 구현 기준의 AI 계층 책임과 HTTP 계약을 설명한다. 핵심은 AI가 Binance 제출 권한이 없는 판단/trace 서비스라는 사실을 분명히 유지하는 것이다.
...
- BE는 그 뒤 `normalized_order_intent` 를 실제 주문 요청으로 변환하고, deterministic 재검증 후에만 Binance Testnet에 제출한다.
