...
FE는 사용자 입력과 결과 시각화에 집중한다. FE는 직접 시장 판단을 하지 않으며, 거래소를 직접 호출하지 않는다. 모든 데이터는 BE API를 통해 수신한다.
...
- 실시간 스트리밍 대신 수동 새로고침 또는 짧은 폴링으로 시작한다.

## 결정 필요 사항

- **MVP 기준 제안**으로 대시보드와 정책 화면을 분리할지, 단일 홈 화면에 통합할지 확정이 필요하다.
- **MVP 기준 제안**으로 실시간 갱신을 폴링으로 할지, 수동 새로고침으로 할지 확정이 필요하다.
- **MVP 기준 제안**으로 리포트 화면을 별도 페이지로 둘지, 대시보드 하단 섹션으로 둘지 확정이 필요하다.
